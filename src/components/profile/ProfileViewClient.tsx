...
export default function ProfileViewClient({
  ...
  const [userProfile, setUserProfile] = useState<IUser | null>(initialProfile);
  const [userTrips, setUserTrips] = useState<ITrip[]>(initialTrips);
  const [profileUsername, setProfileUsername] = useState<string>(username);
  const [followersCount, setFollowersCount] = useState<number>(initialProfile?.followersCount || 0);
  const [followingCount, setFollowingCount] = useState<number>(initialProfile?.followingCount || 0);
  const [tripsCount, setTripsCount] = useState<number>(initialTrips.length || initialProfile?.visitedCount || 0);
  const [isFollowing, setIsFollowing] = useState<boolean>(false);
  const [submittingFollow, setSubmittingFollow] = useState<boolean>(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [loading, setLoading] = useState(!initialProfile);
  const [tripsLoading, setTripsLoading] = useState(initialTrips.length === 0 && !initialProfile);
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-10 flex flex-col md:flex-row items-center md:items-end justify-between gap-6">
          <div className="flex flex-col md:flex-row items-center md:items-end space-y-4 md:space-y-0 md:space-x-6 text-center md:text-left">
            <Image
              ...
              alt={activeUser?.name ? `${activeUser.name}'s profile photo` : 'Traveller Profile Photo'}
              width={112}
              height={112}
              priority
                    ...
                    >
                      <div className="flex items-center space-x-3 min-w-0">
                        <Image
                          ...
                          alt={u.fullName || u.username ? `${u.fullName || u.username}'s avatar` : 'User avatar'}
                          width={40}
                          height={40}
