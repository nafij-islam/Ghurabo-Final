...
export async function POST(request: Request) {
  ...
  try {
    // Stage 1: Read Body & Validate ID Token Presence
    let idToken: string;
    try {
      const body = await request.json();
      idToken = body.idToken;
    } catch (e) {
      console.error('❌ Stage 1 Failed: Unable to parse JSON body');
      return NextResponse.json({ success: false, error: 'Invalid JSON request payload.' }, { status: 400 });
    }

    if (!idToken || typeof idToken !== 'string') {
      console.error('❌ Stage 1 Failed: ID token is missing or not a string');
      return NextResponse.json({ success: false, error: 'Firebase ID token is required.' }, { status: 400 });
    }

    // Stage 2: Server-side Firebase ID token verification
    let decodedToken: any;
    try {
      decodedToken = await adminAuth.verifyIdToken(idToken);
      console.log(`✓ Stage 2 Passed: Google token verified for UID: ${decodedToken.uid}`);
    } catch (authErr: any) {
      console.error('❌ Stage 2 Failed: verifyIdToken failed with error:', authErr?.message || authErr);
    ...
    if (!email) {
      console.error('❌ Stage 2 Failed: Decoded token contains no email address');
      return NextResponse.json(
    ...
    // Stage 3: MongoDB Atlas Connection & User Matching / Creation
    ...
    try {
      ...
      if (conn) {
        console.log(`✓ Stage 3: Searching MongoDB Atlas for email: ${cleanEmail}`);
        let atlasUser = await UserModel.findOne({ email: cleanEmail });

        if (atlasUser) {
          console.log(`✓ Stage 3: Existing MongoDB user found (ID: ${atlasUser.id || atlasUser._id})`);
          ...
          console.log(`✓ Stage 3: Creating new MongoDB user for email: ${cleanEmail}`);
        ...
        console.warn('⚠️ Stage 3: Database fallback to memory store');
      ...
      console.error('⚠️ Database operation encountered error, using memory fallback:', dbErr?.message || dbErr);
      const db = getMemoryDb();
      let memUser = db.users.find((u) => u.email.toLowerCase().trim() === cleanEmail);
      if (!memUser) {
        memUser = {
    ...
    // Stage 4: Session Creation & Cookie Response
    ...
    console.log(`✓ Stage 4 Passed: JWT issued for user ${userObj.email} (ID: ${mongoUserId})`);
    ...
    console.error('❌ Unhandled Fatal Error in /api/auth/google:', fatalError);
    return NextResponse.json(
      { success: false, error: fatalError?.message || 'Authentication service error. Please try again.' },
      { status: 500 }
    );
  }
}
